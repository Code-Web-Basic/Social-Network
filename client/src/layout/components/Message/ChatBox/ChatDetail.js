...
function ChatDetail(props) {
                ...
                <div>
                    {mess.replyId && 'messReply' in mess ? (
                        <div>
                            ...
                            {mess?.messReply[0]?.message === ' ' && (
                                <img
                                    src={mess?.messReply[0]?.source[0]?.data}
                                    alt="imgmess"
                                    style={{ maxWidth: '150px', maxHeight: '150px', opacity: '0.5' }}
                                />
                            )}
    ...
    const scrollTopHandle = async (inView, entry) => {
        if (inView === true) {
            setPaging(paging + 1);
            console.log(paging);
            const scrollData = await dispatch(scrollMessage({ id: id, paging: paging }));
            console.log(scrollData);
            if (scrollData?.payload?.length !== 0) videoRef.current?.scroll({ top: 1000, behavior: 'smooth' });
        }
    };
    useEffect(() => {
        if (paging === 1) videoRef.current?.scrollIntoView({ block: 'start', behavior: 'auto' });
    });
    return (
        <Stack direction="column" height="100%" width="100%">
            ...
            <Stack style={{ width: '100%', height: 'calc(100% - 50px)', position: 'relative' }}>
                ...
                <div style={{ overflowY: 'auto', overflowX: 'hidden', height: 'calc(100% - 75px)' }}>
                    ...
                    {messageData?.map((mess, index) => {
                        return messageData.length - 1 === index ? (
                            <div
                                ref={videoRef}
                                key={mess._id}
                                className={currentUser?._id === mess?.sourceId ? 'item-mess mychat' : 'item-mess'}
                            >
                                {currentUser?._id === mess?.sourceId ? (
                                    <div className="iconReply">
                                        <button onClick={() => handleReplyMessage(mess)}>
                                            <ArrowBendUpLeft style={{ cursor: 'pointer' }} size={20} />
                                        </button>
                                    </div>
                                ) : (
                                    ''
                    ...
                    {(currentUser?._id === messageReceive?.ownerId || JSON.stringify(messageReceive) === '{}') ? (
                        ...
                        <div className="item-mess">
                            ...
                            <div className="iconReply">
                                <button onClick={() => handleReplyMessage(messageReceive.message)}>
                                    <ArrowBendUpLeft style={{ cursor: 'pointer' }} size={20} />
                                </button>
                            </div>
                        </div>
                ...
                >
                    <div className={reply ? 'reply' : 'non-reply'}>
                        <div style={{ fontSize: '12px' }}>
                            {currentUser?._id === replyValue?.sourceId ? (
                                <p>
                                    Đang trả lời <b>Bạn</b>
                                </p>
                            ) : (
                                <p>
                                    Đang trả lời <b>{userfriend?.Name}</b>
                                </p>
                            )}
                            <p>{replyValue?.message === ' ' ? 'Hình ảnh' : replyValue?.message}</p>
                        </div>
                        <div>
                            <button onClick={handleCloseReply} style={{ cursor: 'pointer' }}>
                                <X fontSize="15px" />
                            </button>
                        </div>
                    </div>
                    ...
                    >
                        <Input
                            style={{ fontSize: '18px', width: '100%', fontFamily: '-moz-initial' }}
                            startAdornment={
                        ...
                        {isPickerVisible ? (
                            <div tabIndex="-1" style={{ position: 'absolute', bottom: '30px' }}>
